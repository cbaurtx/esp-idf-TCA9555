...
/* shared variables (with main processors) */
uint32_t i2c_err = I2C_OK;
uint32_t count_m = 0x20000;  // count is unsigned and 18 bit wide. Set to middle
uint32_t btn_data = 0;
uint32_t evt = 0;
...
int main(void) {
  ...
  uint32_t rot_data = 0;
  uint32_t prev_rot_data = 0;

  uint32_t btn_prev_data = 0;

  bool was_pushed = false;

  /* invert all inputs as contacts are connected to ground */
  i2c_write_slave_register(CONFIG_I2C_SLAVE_ADR, INV_PORT0, 0xff);
  i2c_write_slave_register(CONFIG_I2C_SLAVE_ADR, INV_PORT1, 0xff);
  ...
  for(;;) {
    ...
    if (data != prev_data) {
      ...
      #endif
      rot_data = data & 0x0003;
      btn_data = (data >> 2) & 0x7fff;

      if (rot_data != prev_rot_data) {   // may include invalid transitions
        #if CONFIG_AB_ENCODER
        ...
        #else
          // two buttons connected to port 0.0 and 0.1 emulate rot encoder
          if (prev_rot_data == 0) {         // btn 1 and 2 were released
            DELAY(CONFIG_WAIT_DEBOUNCE_SET);
            if (rot_data == (i2c_read_2_slave_registers(CONFIG_I2C_SLAVE_ADR,
        ...
        #endif
        prev_rot_data = rot_data;
        } else {

        // a button changed, wait for debounce and check if still changed
        DELAY(CONFIG_WAIT_DEBOUNCE_SET);
        if (was_pushed) {            // button now needs to be released
          if (0 == (i2c_read_2_slave_registers(CONFIG_I2C_SLAVE_ADR,
            ...
            if ((btn_data << 2) == (i2c_read_2_slave_registers(CONFIG_I2C_SLAVE_ADR,
                                  INPUT_PORT0) & 0xfffc)) {
              evt = BTN_PUSHED;
              was_pushed = true;
              ulp_riscv_wakeup_main_processor();
